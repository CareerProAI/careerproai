    ...
    <Modal scrollable onClose={onClose} labelledBy="application-package-title">
      <div className="flex justify-between items-center border-b border-outline-variant/40 pb-4 mb-4">
        <div>
          ...
          <p className="text-xs text-on-surface-variant">Tailored CV &amp; cover letter for {job.title} at {job.company}</p>
        </div>
      ...
      {profileIncomplete ? (
        <div className="glass-card rounded-2xl p-8 text-center border border-primary/30 bg-primary/5">
          ...
          <p className="text-sm font-bold text-on-surface">Complete your CV profile first</p>
          <p className="text-xs text-on-surface-variant mt-1 mb-4 max-w-sm mx-auto">
            Add your contact email and at least one work experience entry to your CV profile before generating tailored application materials.
          </p>
        ...
        <div className="space-y-3 py-6">
          ...
          <p className="text-xs text-on-surface-variant text-center pt-2">
            Generating your tailored CV and cover letter — this can take up to a minute...
          </p>
        </div>
      ) : error ? (
